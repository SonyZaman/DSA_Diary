...
7. [O(N) and O(N)  Quadratic & Cubic](#7-on-and-on--quadratic--cubic)
...
13. [O(N) Space Complexity](#13-on-space-complexity-1)
14. [Checking Primality  O(N), O(N/2), O(N)](#14-checking-primality--on-on2-on)
...
17. [Recursion  O(2) Time & O(N) Space (Fibonacci)](#17-recursion--o2-time--on-space-fibonacci)
18. [Recursion  O(N) Time & O(N) Space](#18-recursion--on-time--on-space)
...
- In interviews and competitive programming, knowing time complexity ensures your solution runs within the time limit (usually **12 seconds**).

**Why it matters:**
- An O(N) solution might pass for `N = 1,000` but fail for `N = 100,000`.
...
| **(f(n))** | Big Theta | **Average-case**  tight bound (both upper & lower) |
| **(f(n))** | Big Omega | **Best-case**  lower bound on growth |
...
As `n  `, the dominant term overwhelms all others. For example, when `n = 1,000,000`:
- `N = 10`
- `N  = 10`

`N` is completely negligible compared to `N`.
...
2 = N
log(2) = log(N)
K = log(N)        [since log(a) = x]
```

So after **K = log(N)** steps, the search is done  **O(log N)** 
...
Using the geometric series formula `S = a  (r  1) / (r  1)`:
```
S = 1  (2^log(N)  1) / (2  1)
  = 2^log(N)  1
  = N  1          [since 2^log(N) = N]
...
int main() {
    int n;
    cin >> n;
    int a[n][n];                       // NN 2D array  O(N) space
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            a[i][j] = i + j;
        }
...
```
**Why O(N) space?** The 2D array has `n  n = n` cells.

---

## 14. Checking Primality  O(N), O(N/2), O(N)
...
bool isPrime(int n) {
    if (n <= 1) return false;
    for (int i = 2; i * i <= n; i++) {   // Only checks up to n
        if (n % i == 0) return false;
    }
    return true;
}
```
**Why O(N)?** If `n` has a factor > n, then its paired factor must be < n  so we would have already found it. We only need to check up to n.
...
### Classic Pattern  Linear  Logarithmic
```cpp
#include <bits/stdc++.h>
using namespace std;
int main() {
    ...
    for (int i = 1; i <= n; i++) {        // O(N)  runs N times
        for (int j = 1; j <= n; j *= 2) { // O(log N)  halves each iteration? No, doubles
            cout << j << endl;
        }
    }
    return 0;
...
## 17. Recursion  O(2) Time & O(N) Space (Fibonacci)
...
- **Time:** At each level, calls roughly double  **O(2)**
...
## 18. Recursion  O(N) Time & O(N) Space
...
- **Time:** `N + (N-1) + ... + 1 = N(N+1)/2`  **O(N)**
- **Space:** All frames on stack simultaneously have arrays: N + (N-1) + ... + 1  **O(N)**
...
Each group contributes < 1, and there are **log(N) groups**  H(N)  log(N)
...
int main() {
    int n;
    cin >> n;
    for (int i = n; i >= 1; i /= 2) {       // Outer: log(N) times, i = N, N/2, N/4...
        for (int j = 1; j <= i; j++) {       // Inner: runs i times
            cout << j << endl;
        }
    }
...
|----------|----------------------|------------------|
| **O(g(n))** | ` c, n` such that `f(n)  cg(n)` for all `n  n` | g(n) is an **upper bound**  worst it gets |
| **(g(n))** | ` c, n` such that `f(n)  cg(n)` for all `n  n` | g(n) is a **lower bound**  best it gets |
| **(g(n))** | Both O(g(n)) AND (g(n)) | g(n) is a **tight bound**  exact growth rate |
...
| (N log N) | N log N | Even in best case, won't be faster than N log N |
| (N log N) | N log N | Always exactly N log N  tight bound |

### Example: Bubble Sort

| Notation | Value | Meaning |
|----------|-------|---------|
| O(N) | N | Worst case (reverse sorted) |
| (N) | N | Best case (already sorted  just one pass) |
|   | N/A | Not a tight bound since best  worst |

### Quick Memory Aid

```
 = Best case (Omega  "Optimistic")
O = Worst case (O  "Oh no, worst scenario")
 = Average / tight case (Theta  "THink of it as the True complexity")
```
...
**Polynomial time** (practical): O(N), O(N), O(N log N), O(N) for some fixed k.

**Exponential time** (impractical for large N): O(2), O(N!), O(N)

#### Concrete Impact (computer doing 10 ops/sec):
...
#### Sudoku Example:
- **Verify** a completed Sudoku: Check all rows, columns, 33 boxes = 243 checks  **O(N)** 
- **Solve** an empty Sudoku: Up to 9 combinations for 30 empty cells  **exponential** 

> **P  NP**  Every P problem is also in NP (if you can solve it fast, you can verify it fast too). The open question is whether P = NP or P  NP.
...
- Possible routes = (N1)!  O(N!) brute force
- `N=20`: 19!  1.2  10 routes  impossible brute force!
...
**4. Sudoku**
- As described above  NP-Complete for NN grids.
...
- **Merge Sort vs Quick Sort:** Merge Sort = guaranteed O(N log N) + stable, but O(N) space. Quick Sort = O(N log N) average + O(log N) space, but O(N) worst case.
- **Bubble/Selection/Insertion:** All O(N) average/worst, but Insertion Sort is O(N) best case (nearly sorted).
...
| **Disjoint Set (Union-Find)** | Group membership, connected components | Find, Union | O((N))  O(1) amortized |
...
|-----------|-------------|-------|-------------|
| Brute Force | O(N) or O(2) | O(1) | Small N, as a starting point |
| Greedy | O(N log N) | O(1)O(N) | Locally optimal  globally optimal |
| Dynamic Programming | O(N) or O(NW) | O(N)O(N) | Overlapping subproblems + optimal substructure |
| Divide & Conquer | O(N log N) | O(log N)O(N) | Split problem, solve parts independently |
| Backtracking | O(2) or O(N!) | O(N) | Explore all possibilities, prune invalid ones |
| Two Pointers | O(N) | O(1) | Sorted array, find pairs/triplets |
| Sliding Window | O(N) | O(1)O(k) | Subarray/substring problems of fixed/variable size |
...
| Longest Palindromic Subsequence | O(N) | O(N) | Longest palindrome in a string |
| Matrix Chain Multiplication | O(N) | O(N) | Min cost to multiply matrices |
| Subset Sum | O(N  sum) | O(N  sum) | Can a subset sum to target? |
| Rod Cutting | O(N) | O(N) | Max value from cutting a rod |
...
> **10 Rule:** C++ processes ~10 (100 million) operations per second. Use this to map `n` to the required complexity.

| Input Size (n) | Maximum Acceptable Complexity | Why |
|----------------|-------------------------------|-----|
| n  1012 | O(N!) | 12!  5 crore operations |
| n  2025 | O(2) | 2  3.3 crore operations |
| n  500 | O(N) | 500 = 12.5 crore operations |
| n  10,000 | O(N) | 10,000 = 10 crore operations |
...
3. **Plan accordingly**  don't write an O(N) solution if N = 10.

**Example:**
- Problem says `1  N  10`  O(N) = 10 = too slow. Need **O(N log N)** or better.
- Problem says `1  N  20`  O(2) = 10 = fine. Use **bitmask DP or backtracking**.
- Problem says `1  N  10`  Need a **single-pass O(N)** solution. Even O(N log N) may be tight.
...
```
O(1) < O(log N) < O(N) < O(N) < O(N log N) < O(N) < O(N) < O(2) < O(N!) < O(N)
```
...
| O(N) | Root | Prime checking (optimized) |
| O(N) | Linear | Single loop, Linear Search |
| O(N log N) | Linearithmic | Merge Sort, Heap Sort |
| O(N) | Quadratic | Bubble Sort, nested loops |
| O(N) | Cubic | Matrix multiplication (naive) |
| O(2) | Exponential | Fibonacci (naive), all subsets |
...
| Recursion makes 2 calls (no memo) | O(2) |
| Recursion makes N calls, each O(N) | O(N) |
...
| Recursion depth N, each level O(N) | O(N) total |
...
- **"Why does binary search work in O(log N)?"**  Each step halves the search space. After log(N) halvings, one element remains.
